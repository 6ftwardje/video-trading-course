...
export async function PATCH(request: NextRequest, ctx: { params: { id: string } }) {
  try {
    ...
    if (id === null) {
      return NextResponse.json({ error: 'Invalid practical lesson id' }, { status: 400 })
    }
    ...
    if (error) {
      console.error('PATCH practical_lesson content', error)
      return NextResponse.json({ error: 'Error updating practical lesson' }, { status: 500 })
    }

    if (!data) {
      return NextResponse.json({ error: 'Practical lesson not found' }, { status: 404 })
    }
    ...
    console.error('PATCH /api/admin/content/practical-lessons/[id]', err)
